...
The complete build process takes about 15 minutes (depending on your internet connection).
...
Just [copy](a "scp kuba-setup-1.28.0.tgz.self root@192.168.178.53:~") the previously created setup package to a new Alpine Linux server and start it with the parameters "init single".
...
### Build
First you build the KubA app package. It then contains all necessary artefacts for the installation of wordpress, minio and minecraft in an airgap environment.
...
### Install
Then [copy](a "scp kuba-apps-2023.9.tgz.self root@192.168.178.53:~") the previously created app package to your kubernetes node and install the desired application.
